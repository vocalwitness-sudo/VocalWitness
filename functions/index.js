...
// ======================================================
// RESTRICTED CORS HELPER (Production-safe)
// ======================================================
      ...
      try {
        ...
        if (
          ...
          return res.status(400).json({
            error: "fileName and fileType must be non-empty strings.",
          });
        }
  ...
  async (request) => {
    ...
    if (!isMod) {
      throw new HttpsError(
        "permission-denied",
        "Only moderators or admins can perform moderated deletes."
      );
    }
    ...
    if (
      ...
      throw new HttpsError(
        "invalid-argument",
        "collection and docId must be valid strings."
      );
    }
    ...
    if (!allowedCollections.includes(collection)) {
      throw new HttpsError(
        "invalid-argument",
        "Collection not allowed for moderated deletion."
      );
    }
    ...
    try {
      ...
      await writeAuditLog({
        ...
        details: {
          ...
          originalAuthor:
            originalData.authorId ||
            originalData.ownerId ||
            originalData.witnessId ||
            null,
          contentPreview: originalData.content
            ? String(originalData.content).substring(0, 200)
            : null,
        },
        severity: "high",
      });
...
async function analyzeToxicityWithPerspective(content = "") {
  ...
  try {
    ...
    const isToxic =
      toxicityScore > 0.7 || insultScore > 0.7 || threatScore > 0.6;
...
function gentleModerationCheck(content = "") {
  ...
  return {
    ...
    note: isFlagged
      ? "Flagged by local fallback moderation check."
      : "Passed local fallback check.",
  };
}
...
exports.initializePaystack = onCall(
  ...
  async (request) => {
    ...
    if (typeof amount !== "number" || amount < 1000) {
      throw new HttpsError(
        "invalid-argument",
        "Amount must be a number equal to or greater than 1000."
      );
    }
...
exports.generateZKProof = onCall(
  ...
  async (request) => {
    if (!request.auth) {
      throw new HttpsError(
        "unauthenticated",
        "Authentication required to generate ZK proof."
      );
    }
    ...
    try {
      ...
      if (!fs.existsSync(wasmPath) || !fs.existsSync(zkeyPath)) {
        throw new HttpsError(
          "failed-precondition",
          "Circuit artifacts (.wasm / .zkey) missing on server."
        );
      }
      ...
      throw new HttpsError(
        "internal",
        error.message || "Failed to generate ZK proof on server."
      );
    }
  }
);
...
exports.verifyZKProof = onCall(
  ...
  async (request) => {
    if (!request.auth) {
      throw new HttpsError(
        "unauthenticated",
        "Authentication required to submit ZK proof."
      );
    }
    ...
    if (!proof || !publicSignals) {
      throw new HttpsError(
        "invalid-argument",
        "Missing proof or public signals payload."
      );
    }
    ...
    try {
      ...
      if (!fs.existsSync(keyPath)) {
        throw new HttpsError(
          "failed-precondition",
          "Verification key file not found on server."
        );
      }
      ...
      throw new HttpsError(
        "internal",
        error.message || "Failed to verify ZK proof."
      );
    }
  }
);
...
exports.verifyMediaPipeline = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      console.log(
        `🔍 Starting media forensic pipeline for post ${postId} (${mediaUrl})`
      );
